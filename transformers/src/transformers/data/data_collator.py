...
class DataCollatorForDialogGeneration:
    ...
    def __call__(
        ...
        for example in examples:
            print('this is where the two many values to unpack error happned ', len(example))
        # Assumes the input is a list of tuples where each tuple contains (history, response)
        histories, responses = zip(*examples)
        for example in examples:
            print('this is where the two many values to unpack error happned ', len(example))
        if self.mlm:
            inputs, labels = self.mask_tokens(histories)
            return {"input_ids": inputs, "labels": labels}
        # Format the batches based on the specified mode
        else :
            if self.format_mode == 'cat':
                batch = self._tensorize_batch(histories)
                labels = self._tensorize_batch(responses)
            elif self.format_mode == 'peek':
                batch = self._tensorize_batch(responses)
                labels = batch.clone()
            else:
    ...
    """ def _tokenize_and_concatenate(self, histories: List[List[int]], responses: List[List[int]]) -> torch.Tensor:
        # Tokenize history and response separately
        tokenized_histories = [self.tokenizer(hist, add_special_tokens=False)["input_ids"] for hist in histories]
        tokenized_responses = [self.tokenizer(resp, add_special_tokens=False)["input_ids"] for resp in responses]

        # Concatenate history and response
        inputs = [torch.tensor(hist + self.tokenizer.eos_token_id + resp, dtype=torch.long) for hist, resp in zip(tokenized_histories, tokenized_responses)]
